"""Preflight checks for Home Assistant timer tool."""
...
def home_assistant_timer_prepare(
    args: dict[str, Any],
    *,
    start_time: float,
    ...
    if action not in {"state", "start", "pause", "cancel", "finish"}:
        ...
        return None, {"content": [{"type": "text", "text": "Action must be one of: state, start, pause, cancel, finish."}]}
    if not entity_id:
