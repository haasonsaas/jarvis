...
class TestConfig:
    ...
    def test_whitespace_only_openai_key_raises(self, monkeypatch):
    ...
    def test_required_env_value_is_trimmed(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "  sk-trimmed  ")

        from jarvis.config import Config
        c = Config()
        assert c.openai_api_key == "sk-trimmed"
    ...
    def test_defaults(self, config):
        ...
        assert config.openai_router_model == config.openai_model
        assert config.router_timeout_sec == 2.0
        assert config.policy_router_min_confidence == 0.55
        assert config.interruption_router_timeout_sec == 1.5
        assert config.interruption_resume_min_confidence == 0.6
        assert config.semantic_turn_enabled is True
        assert config.semantic_turn_router_timeout_sec == 0.8
        assert config.semantic_turn_min_confidence == 0.6
        assert config.semantic_turn_extension_sec == 0.6
        assert config.semantic_turn_max_transcript_chars == 220
    ...
    def test_invalid_memory_embedding_settings_raise(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "test")
        from jarvis.config import Config
        with pytest.raises(ValueError, match="memory_embedding_vector_weight"):
            Config(memory_embedding_vector_weight=1.5)
        with pytest.raises(ValueError, match="memory_embedding_min_similarity"):
            Config(memory_embedding_min_similarity=-0.1)
        with pytest.raises(ValueError, match="memory_embedding_timeout_sec"):
            Config(memory_embedding_timeout_sec=0.0)
        with pytest.raises(ValueError, match="memory_conflict_resolution_timeout_sec"):
            Config(memory_conflict_resolution_timeout_sec=0.0)
    ...
    def test_startup_warnings_include_invalid_optional_env_values(self, monkeypatch):
        ...
        monkeypatch.setenv("INTERRUPTION_ROUTER_TIMEOUT_SEC", "0")
        monkeypatch.setenv("INTERRUPTION_RESUME_MIN_CONFIDENCE", "nan")
        monkeypatch.setenv("SEMANTIC_TURN_ROUTER_TIMEOUT_SEC", "0")
        monkeypatch.setenv("SEMANTIC_TURN_MIN_CONFIDENCE", "nan")
        monkeypatch.setenv("SEMANTIC_TURN_EXTENSION_SEC", "0")
        monkeypatch.setenv("SEMANTIC_TURN_MAX_TRANSCRIPT_CHARS", "oops")
        ...
        monkeypatch.setenv("SEMANTIC_TURN_ENABLED", "later")
        ...
        assert "INTERRUPTION_ROUTER_TIMEOUT_SEC invalid" in text
        assert "INTERRUPTION_RESUME_MIN_CONFIDENCE invalid" in text
        assert "SEMANTIC_TURN_ROUTER_TIMEOUT_SEC invalid" in text
        assert "SEMANTIC_TURN_MIN_CONFIDENCE invalid" in text
        assert "SEMANTIC_TURN_EXTENSION_SEC invalid" in text
        assert "SEMANTIC_TURN_MAX_TRANSCRIPT_CHARS invalid" in text
        ...
        assert "SEMANTIC_TURN_ENABLED invalid boolean" in text
    ...
    def test_timeout_env_values_fall_back_to_defaults_when_invalid(self, monkeypatch):
        ...
        monkeypatch.setenv("TURN_TIMEOUT_ACT_SEC", "0")
        monkeypatch.setenv("ROUTER_TIMEOUT_SEC", "0")
        monkeypatch.setenv("INTERRUPTION_ROUTER_TIMEOUT_SEC", "0")
        monkeypatch.setenv("SEMANTIC_TURN_ROUTER_TIMEOUT_SEC", "0")
        monkeypatch.setenv("SEMANTIC_TURN_EXTENSION_SEC", "0")
        from jarvis.config import Config
        ...
        assert c.router_timeout_sec == 2.0
        assert c.interruption_router_timeout_sec == 1.5
        assert c.semantic_turn_router_timeout_sec == 0.8
        assert c.semantic_turn_extension_sec == 0.6
    ...
    def test_timeout_env_values_can_be_set(self, monkeypatch):
        ...
        monkeypatch.setenv("ROUTER_TIMEOUT_SEC", "1.75")
        monkeypatch.setenv("INTERRUPTION_ROUTER_TIMEOUT_SEC", "0.9")
        monkeypatch.setenv("SEMANTIC_TURN_ROUTER_TIMEOUT_SEC", "0.65")
        monkeypatch.setenv("SEMANTIC_TURN_MIN_CONFIDENCE", "0.72")
        monkeypatch.setenv("SEMANTIC_TURN_EXTENSION_SEC", "1.1")
        monkeypatch.setenv("SEMANTIC_TURN_MAX_TRANSCRIPT_CHARS", "180")
        from jarvis.config import Config
        ...
        assert c.router_timeout_sec == 1.75
        assert c.interruption_router_timeout_sec == 0.9
        assert c.semantic_turn_router_timeout_sec == 0.65
        assert c.semantic_turn_min_confidence == 0.72
        assert c.semantic_turn_extension_sec == 1.1
        assert c.semantic_turn_max_transcript_chars == 180

    def test_policy_router_min_confidence_bounds_are_validated(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "test")
        from jarvis.config import Config

        with pytest.raises(ValueError, match="policy_router_min_confidence"):
            Config(policy_router_min_confidence=-0.1)
        with pytest.raises(ValueError, match="policy_router_min_confidence"):
            Config(policy_router_min_confidence=1.1)

    def test_interruption_resume_min_confidence_bounds_are_validated(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "test")
        from jarvis.config import Config

        with pytest.raises(ValueError, match="interruption_resume_min_confidence"):
            Config(interruption_resume_min_confidence=-0.1)
        with pytest.raises(ValueError, match="interruption_resume_min_confidence"):
            Config(interruption_resume_min_confidence=1.1)

    def test_semantic_turn_min_confidence_bounds_are_validated(self, monkeypatch):
        monkeypatch.setenv("OPENAI_API_KEY", "test")
        from jarvis.config import Config

        with pytest.raises(ValueError, match="semantic_turn_min_confidence"):
            Config(semantic_turn_min_confidence=-0.1)
        with pytest.raises(ValueError, match="semantic_turn_min_confidence"):
            Config(semantic_turn_min_confidence=1.1)
